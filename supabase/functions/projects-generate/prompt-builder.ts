...
<div style="position: fixed; bottom: 0; left: 0; right: 0; background: rgba(255, 255, 255, 0.95); backdrop-filter: blur(8px); border-top: 1px solid rgba(0, 0, 0, 0.08); z-index: 9999; padding: 12px 16px;">
  <div style="display: grid; grid-template-columns: 1fr auto; align-items: center; max-width: 1200px; margin: 0 auto; gap: 12px;">
    <!-- Text + Logo Column -->
    <div style="font-size: 11px; line-height: 1.3; color: #6b7280; display: flex; align-items: center; gap: 6px; flex-wrap: wrap;">
      ${footerContent}
    </div>
    <!-- Button Column -->
    <div style="flex-shrink: 0;">
      <a href="https://sandpix-harada.vercel.app/" target="_blank" style="display: inline-flex; align-items: center; gap: 4px; background: linear-gradient(135deg, #3b82f6, #8b5cf6); color: white; text-decoration: none; padding: 6px 12px; border-radius: 16px; font-size: 11px; font-weight: 600; transition: all 0.2s ease; box-shadow: 0 2px 6px rgba(59, 130, 246, 0.25); white-space: nowrap;">
        ${footerConfig.cta}
...
<style>
  ...
  private buildFooterContent(footerConfig: any, language: Language): string {
    const sandpixBrand = `
      <span style="display: inline-flex; align-items: center; gap: 3px; font-weight: 700; background: linear-gradient(135deg, #3b82f6, #8b5cf6, #ef4444, #f59e0b); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text; font-size: 12px;">
        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="url(#gradient)" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" style="margin-right: 1px;">
          <defs>
